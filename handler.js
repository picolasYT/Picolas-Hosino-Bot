...
// --- UTILIDAD: Normalización de JID ---
function jidNormalizedUser(jid = '') {
  if (!jid) return ''
  // Si ya tiene @lid o @s.whatsapp.net lo respetamos, si no, lo agregamos
  if (jid.endsWith('@lid') || jid.endsWith('@s.whatsapp.net')) return jid
  jid = jid.replace(/[^0-9]/g, '')
  return jid ? jid + '@s.whatsapp.net' : ''
}
...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    // --- ADAPTACIÓN PARTICIPANTES Y CITADOS PARA @lid/@s.whatsapp.net ---
    let partisipan = m.key?.participant || ''
    if (partisipan.endsWith('@lid')) {
      const parsingParticipant = m.metadata?.participants?.find(p => p.id === partisipan)
      m.participant = parsingParticipant?.jid || partisipan
    } else {
      m.participant = partisipan
    }

    let quotePartisipan = jidNormalizedUser(m.msg?.contextInfo?.participant || m.quoted?.from)
    if (quotePartisipan.endsWith('@lid')) {
      const quotedparsingParticipant = m.metadata?.participants?.find(p => p.id === quotePartisipan)
      if (m.quoted) m.quoted.sender = quotedparsingParticipant?.jid || quotePartisipan
    } else {
      if (m.quoted) m.quoted.sender = quotePartisipan
    }
    ...
    try {
      ...
      if (chat) {
        ...
        detect: true, primaryBot: null, antiBot: false, antiBot2: false, modoadmin: false,
    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        let extra = {
          ...
          participants, groupMetadata, user, bot, isROwner, isOwner, isRAdmin, isAdmin,
