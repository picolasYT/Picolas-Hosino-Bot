...
// === FUNCIÓN CLAVE PARA RESOLVER JID REAL (SOPORTE @lid) ===
function getRealJid(jid, metadata) {
    if (!jid) return ''
    if (jid.endsWith('@lid')) {
        const part = metadata?.participants?.find(p => p.id === jid)
        return part?.jid || jid
    }
    return jid
}
...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        // === OBTENER METADATOS DE GRUPO SI ES GRUPO ===
        const groupMetadata = m.isGroup
            ? ((this.chats[m.chat] || {}).metadata || await this.groupMetadata(m.chat).catch(_ => null)) || {}
            : {}
        const participants = m.isGroup ? groupMetadata.participants || [] : []

        // === RESOLVER EL JID REAL DEL EMISOR Y CITADOS ===
        m.sender = getRealJid(m.sender, groupMetadata)
        // Para mensajes de grupo, participant debe resolverse igual
        m.participant = getRealJid(m.key?.participant || m.participant, groupMetadata)
        // Para citados
        if (m.quoted) {
            m.quoted.sender = getRealJid(
    ...
    // === ROLES (USANDO JID REAL) ===
    const isROwner = [this.decodeJid(this.user.id), ...global.owner.map(([number]) => number)]
    ...
    // === BUSCAR ADMIN/BOT EN PARTICIPANTES USANDO JID REAL ===
    const user = m.isGroup ? participants.find(u => getRealJid(u.id, groupMetadata) === m.sender) : {} || {}
    const bot = m.isGroup ? participants.find(u => getRealJid(u.id, groupMetadata) == this.user.jid) : {} || {}
    ...
    for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
            ...
            try {
                ...
                if (m.coin)
                    this.reply(m.chat, `❮✦❯ Utilizaste ${+m.coin} ${moneda}`, m)
            }
            break
        }
    }
