...
const isMods = isOwner || global.mods.map(v => v.replace(/[^0-9]/g, '') + '@lid').includes(m.sender)
...
const bot = global.db.data.settings ? global.db.data.settings[conn.user.jid] || {} : {}

const isRAdmin = m.isGroup ? participants.some(p => p.id === m.sender && p.admin !== null) : false
