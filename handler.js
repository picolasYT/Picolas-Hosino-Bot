...
// --- Normalización universal de JID ---
function normalizeJid(jid) {
    if (!jid) return ''
    if (jid.endsWith('@s.whatsapp.net')) return jid
    if (jid.endsWith('@lid')) {
        // Extrae sólo el número y lo pasa a s.whatsapp.net
...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        // --- Normalización de los participantes y quoted participant ---
        if (m.isGroup) {
            m.metadata = m.metadata || (await this.groupMetadata(m.chat).catch(_ => null)) || {}
            m.metadata.participants = m.metadata.participants || []
        }
        // Para m.participant (quien ejecuta el comando)
        let partisipan = m.key?.participant || m.participant || m.sender
        if (partisipan && partisipan.endsWith('@lid')) {
            const parsingParticipant = m.metadata?.participants?.find(p => normalizeJid(p.id) === normalizeJid(partisipan))
            m.participant = parsingParticipant?.jid || partisipan
        } else {
            m.participant = partisipan || m.sender
        }
        // Para m.quoted.sender (usuario citado)
        let quotePartisipan = m.msg?.contextInfo?.participant || m.quoted?.from
        if (quotePartisipan && quotePartisipan.endsWith('@lid')) {
            const quotedparsingParticipant = m.metadata?.participants?.find(p => normalizeJid(p.id) === normalizeJid(quotePartisipan))
            if (m.quoted) m.quoted.sender = quotedparsingParticipant?.jid || quotePartisipan
        } else if (m.quoted && quotePartisipan) {
            m.quoted.sender = quotePartisipan
        }
        ...
        if (user) {
            ...
            global.db.data.users[m.sender] = {
                ...
                registered: false, genre: '', birth: '', marry: '', description: '', packstickers: null, name: m.name,
                age: -1, regTime: -1, afk: -1, afkReason: '', banned: false, useDocument: false, bank: 0, level: 0,
        ...
        if (chat) {
            ...
            global.db.data.chats[m.chat] = {
                ...
                delete: false, autoAceptar: false, autoRechazar: false, detect: true, primaryBot: null,
                antiBot: false, antiBot2: false, modoadmin: false, antiLink: true, antifake: false,
    ...
    // --- Owner/mods/prems, normalizado ---
    const detectwhat = m.sender.includes('@lid') ? '@lid' : '@s.whatsapp.net'
    const isROwner = [...global.owner.map(([number]) => normalizeJid(number + detectwhat))].includes(normalizeJid(m.sender))
    const isOwner = isROwner || m.fromMe
    const isMods = isROwner || global.mods.map(v => normalizeJid(v + detectwhat)).includes(normalizeJid(m.sender))
    const isPrems = isROwner || global.prems.map(v => normalizeJid(v + detectwhat)).includes(normalizeJid(m.sender)) || _user.premium == true
    ...
    // --- Group admin/participant detection universal ---
    const groupMetadata = (m.isGroup ? ((conn.chats[m.chat] || {}).metadata || await this.groupMetadata(m.chat).catch(_ => null)) : {}) || {}
    const participants = (m.isGroup ? groupMetadata.participants : []) || []
    // user: encuentra tu propio info en participantes (ID normalizado)
    const user = (m.isGroup ? participants.find(u => normalizeJid(u.id) === normalizeJid(m.sender)) : {}) || {}
    // bot: encuentra info del bot en participantes (ID normalizado)
    let botJid = (conn.user.lid || conn.user.jid)
    const bot = (m.isGroup ? participants.find(u => normalizeJid(u.id) === normalizeJid(botJid)) : {}) || {}
    ...
    for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
            ...
            let extra = {
                ...
                user, bot, isROwner, isOwner, isRAdmin, isAdmin, isBotAdmin, isPrems, chatUpdate, __dirname: ___dirname, __filename
            }
            try {
