...
function normalizeJid(jid) {
  ...
  if (jid.endsWith('@g.us')) return jid // grupos no se normalizan
  return jid
}
...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    // ---- NORMALIZA SENDER Y PARTICIPANTES ----
    if (m.isGroup) {
    ...
    m.participant = normalizeJid(m.participant)

    // Si hay citado, normalízalo también
    if (m.quoted && m.quoted.sender) m.quoted.sender = normalizeJid(m.quoted.sender)
    // Si hay mención, normalízala también
    ...
    // ... [tu inicialización de propiedades de usuario aquí, igual que antes] ...

    let chat = global.db.data.chats[m.chat]
    if (typeof chat !== 'object') global.db.data.chats[m.chat] = {}
    // ... [tu inicialización de propiedades de chat aquí, igual que antes] ...

    var settings = global.db.data.settings[this.user.jid]
    if (typeof settings !== 'object') global.db.data.settings[this.user.jid] = {}
    // ... [tu inicialización de settings aquí, igual que antes] ...
  } catch (e) {
    console.error(e)
  }
  ...
  const isROwner = global.owner.some(([n]) => normalizeJid(n + '@s.whatsapp.net') === normalizeJid(m.sender))
  const isOwner = isROwner || m.fromMe
  const isMods = isROwner || global.mods.some(n => normalizeJid(n + '@s.whatsapp.net') === normalizeJid(m.sender))
  const isPrems = isROwner ||
    global.prems.some(n => normalizeJid(n + '@s.whatsapp.net') === normalizeJid(m.sender)) ||
    _user?.premium === true
  ...
  const botJid = normalizeJid(this.user.lid || this.user.jid)
  const bot = m.isGroup ? participants.find(u => normalizeJid(u.id) === botJid) : {}
