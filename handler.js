...
// Normalizador universal de JIDs (quita :algo@ y todo lo que no sea el ID principal)
...
if (user) {
    ...
    global.db.data.users[m.sender] = {
        ...
        genre: '', birth: '', marry: '', description: '', packstickers: null, name: m.name,
        age: -1, regTime: -1, afk: -1, afkReason: '', banned: false, useDocument: false,
...
if (partisipan.endsWith('@lid')) {
    ...
    m.participant = partisipan;
}

// LÓGICA LID PARA QUOTED (¡no modificar m.quoted directamente!)
let quotePartisipan = m.msg?.contextInfo?.participant || m.quoted?.from || '';
if (quotePartisipan.endsWith('@lid')) {
    const quotedparsingParticipant = participants.find(p => p.id === quotePartisipan);
    m.quotedSender = quotedparsingParticipant?.jid || quotePartisipan;
} else {
    m.quotedSender = quotePartisipan;
}

// FUNCIONA PARA CUALQUIER FORMATO DE ID
const user = (m.isGroup
    ? participants.find(u => normalizeJid(u.id) === normalizeJid(m.sender))
    : {}
) || {}

const bot = (m.isGroup
    ? participants.find(u => normalizeJid(u.id) === normalizeJid(this.user.jid))
    : {}
) || {}
...
if (opts['queque'] && m.text && !(isMods || isPrems)) {
    ...
    setInterval(async function () {
        if (queque.indexOf(previousID) === -1) return clearInterval(this)
        await delay(time)
    }, time)
}
...
participants, groupMetadata, user, bot, isROwner, isOwner, isRAdmin, isAdmin, isBotAdmin, isPrems,
chatUpdate, __dirname: ___dirname, __filename
}
try {
